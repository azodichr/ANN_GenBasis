...
comp1 = defaultdict(list)
count = 0
for filex in files:
  name = ids[count]
  with open(filex) as f:
    file_list = f.read().splitlines()
  comp1[name] = file_list
  count += 1

# Read in cluster key as dictionary of lists
with open(CLUST, 'r') as clus:
  clus_lines = clus.read().splitlines()

clust = defaultdict(list)
count2 = 0
for c in clus_lines:
  for item in c.strip().split(','):
    clust[item] = 'cluster_' + str(count2)
  count2 += 1

# Convert compare dictionary to clusters
comp = defaultdict(list)
for key in comp1:
  for value in comp1[key]:
    comp[key].append(clust[value])
...
for A in ids:
  for B in ids2:
    if A == B:
      ...
      df[A][B] = len([i for i in comp[A] if i in comp[B]])
  ids2.remove(A)
print(df)
...
if n_comparing == 4:
  print('daugh')
  n1234 = len([i for i in comp[ids[0]] if i in comp[ids[1]] and i in comp[ids[2]] and i in comp[ids[3]]])
  n234 = len([i for i in comp[ids[1]] if i in comp[ids[2]] and i in comp[ids[3]]])
  n134 = len([i for i in comp[ids[0]] if i in comp[ids[2]] and i in comp[ids[3]]])
  n124 = len([i for i in comp[ids[0]] if i in comp[ids[1]] and i in comp[ids[3]]])
  n123 = len([i for i in comp[ids[0]] if i in comp[ids[1]] and i in comp[ids[2]]])
  n12 = len([i for i in comp[ids[0]] if i in comp[ids[1]]])
  n13 = len([i for i in comp[ids[0]] if i in comp[ids[2]]]) 
  n14 = len([i for i in comp[ids[0]] if i in comp[ids[3]]]) 
  n23 = len([i for i in comp[ids[1]] if i in comp[ids[2]]]) 
  n24 = len([i for i in comp[ids[1]] if i in comp[ids[3]]]) 
  n34 = len([i for i in comp[ids[2]] if i in comp[ids[3]]]) 
